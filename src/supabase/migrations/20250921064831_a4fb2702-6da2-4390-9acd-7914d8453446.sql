-- Phase 3: Database Security & Performance Hardening

-- Fix missing indexes for performance optimization
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_micro_modules_course_topic 
ON micro_modules(course_id, topic_index, subtopic_index);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_micro_progress_user_course 
ON user_micro_progress(user_id, course_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_learning_analytics_session 
ON learning_analytics(user_id, course_id, session_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_generation_sessions_user_status 
ON generation_sessions(user_id, status, last_activity);

-- Add missing RLS policies for enhanced_learning_analytics
ALTER TABLE enhanced_learning_analytics ENABLE ROW LEVEL SECURITY;

-- Add data integrity constraints
ALTER TABLE micro_modules 
ADD CONSTRAINT valid_topic_indexes 
CHECK (topic_index >= 0 AND subtopic_index >= 0 AND module_index >= 0);

ALTER TABLE user_micro_progress 
ADD CONSTRAINT valid_quiz_score 
CHECK (quiz_score IS NULL OR (quiz_score >= 0 AND quiz_score <= 100));

ALTER TABLE user_micro_progress 
ADD CONSTRAINT valid_mastery_level 
...
-- Create function for comprehensive data validation
CREATE OR REPLACE FUNCTION validate_data_integrity()
RETURNS TABLE(table_name TEXT, issue_type TEXT, count BIGINT) AS $$
BEGIN
  RETURN QUERY
  SELECT 'user_micro_progress'::TEXT, 'orphaned_progress'::TEXT, 
         COUNT(*) FROM user_micro_progress 
         WHERE micro_module_id NOT IN (SELECT id FROM micro_modules)
  UNION ALL
  SELECT 'learning_analytics'::TEXT, 'orphaned_analytics'::TEXT,
         COUNT(*) FROM learning_analytics 
         WHERE course_id NOT IN (SELECT id FROM courses)
  UNION ALL
  SELECT 'micro_modules'::TEXT, 'invalid_indexes'::TEXT,
         COUNT(*) FROM micro_modules 
         WHERE topic_index < 0 OR subtopic_index < 0 OR module_index < 0;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;